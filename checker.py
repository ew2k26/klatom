...
# ---------------------------------------------------------------------------
# SSL/Proactor monkey-patches for Windows aiohttp compatibility.
# On Windows, the Proactor event loop raises noisy RuntimeError/ConnectionResetError
# from SSL protocol internals during normal operation. These patches suppress
# those harmless exceptions to keep logs clean without affecting functionality.
# ---------------------------------------------------------------------------
...
def parse_args() -> AppSettings:
    ...
    parser.add_argument("--version", action="version", version=f"ew² v{VERSION}")
...
async def _run_checker(cfg: RunConfig, settings: AppSettings) -> None:
    ...
    if cfg.scraped and len(cfg.proxies) > 10:
        ...
        console.print()
        working_proxies = await pm.apply_speed_results(results, remove_slow=True, max_latency_ms=10000)
        speed_test_result(results)

        if working_proxies == 0:
            console.print(f"[{C.WARNING}]No fast proxies found — using all scraped proxies[/]")
            working_proxies = len(cfg.proxies)
        else:
            console.print(f"[{C.SUCCESS}]Using {working_proxies} fast proxies[/]")
        console.print()

    if working_proxies > 0 and cfg.concurrency > working_proxies:
        cfg.concurrency = max(1, working_proxies)
