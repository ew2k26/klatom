...
from config import DATA_DIR as _cfg_data

DATA = _cfg_data
AUTH_FILE = DATA / ".auth"
SESSION_FILE = DATA / ".session"
TOKENS_FILE = DATA / ".tokens"
ACTIVATION_FILE = DATA / ".activation"
RESULTS_DIR = DATA
...
def _show_session():
    ...
    if activation_data:
        ...
        color = C.SUCCESS if status == "ACTIVE" else C.DANGER
        t.add_row("License Status", f"[{color}]{status}[/]")
        if activated_at:
            t.add_row("Activated", f"[{C.MUTED}]{datetime.fromtimestamp(activated_at).strftime('%Y-%m-%d %H:%M')}[/]")
    else:
        t.add_row("License Status", f"[{C.DANGER}]Not activated[/]")
            ...
            async with aiohttp.ClientSession() as sess:
                async with sess.get(f"{url}/config.json", timeout=aiohttp.ClientTimeout(total=10)) as resp:
                    if resp.status == 200:
                        ...
                        console.print(f"  [{C.MUTED}]Local version:  {VERSION}[/]")
                    else:
